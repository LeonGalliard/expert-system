...
class DiagnosaController extends Controller
{
	...
	{ //Fungsi proses merupakan bagian dari sebuah kontroler dan digunakan untuk mengatur tindakan atau logika yang dilakukan saat memproses suatu permintaan yang dikirim oleh pengguna melalui metode POST.
		...
		if ($step == '1') {
			...
			$request->session()->put('nama', $request->post('nama')); // Diubah menjadi request post karena sekarang page diagnosa tidak perlu login dulu untuk dapat diakses
		...
		if ($step == '2') {
			...
			$laporan = new Laporan([
				// 'user_id' => auth()->user()->id, // Karena sekarang user tidak perlu login
				'no_hp' => $no_hp,
				'tahun_motor' => $tahun_motor,
			...
			foreach ($hasil as &$aturan) {
				...
				$kerusakan = Kerusakan::where('kode_kerusakan', $kode_kerusakan)->first(); //berdasarkan kerusakan yang didapat sebelumnya
